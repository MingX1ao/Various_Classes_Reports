...
//parameter define
parameter CLK_FREQ = 125000000;               //系统时钟频率
...
//当uart_tx_en为高时，保存发送数据到临时寄存器tx_data_t，并拉高uart_tx_busy信号
always @(posedge clk or posedge reset) begin
    ...
    //当计数到停止位结束时，停止发送过程
        ...
        tx_data_t <= 8'b0;     //清空发送数据临时寄存器
        uart_tx_busy <= 1'b0;  //并拉低uart_tx_busy信号
    end
    else begin
        tx_data_t <= tx_data_t;
